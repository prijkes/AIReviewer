...
using Quaally.AI;
using Quaally.AI.FunctionParameters;
using Quaally.AzureDevOps.Functions.Parameters;
using Quaally.Options;
using Quaally.Utils;
using System.Text.Json;

namespace Quaally.AzureDevOps.Functions;
    ...
    IOptionsMonitor<ReviewerOptions> options)
{
    ...
    {
        var args = JsonSerializer.Deserialize<UpdatePrDescriptionParameters>(argsJson)
            ?? throw new InvalidOperationException("Failed to deserialize parameters");
    ...
    {
        var args = JsonSerializer.Deserialize<GetPrFilesParameters>(argsJson);
    ...
    {
        var args = JsonSerializer.Deserialize<GetPrDiffParameters>(argsJson)
            ?? throw new InvalidOperationException("Failed to deserialize parameters");
    ...
    {
        var args = JsonSerializer.Deserialize<GetPrCommitsParameters>(argsJson);
